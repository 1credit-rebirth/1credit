...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete 1Credit database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no 1Credit datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old 1Credit datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected 1Credit 0.7 datadir."
    ;;
  3)
    echo "Detected 1Credit pre-0.8 datadir."
    ;;
  4)
    echo "Detected 1Credit 0.8 datadir."
    ;;
esac
