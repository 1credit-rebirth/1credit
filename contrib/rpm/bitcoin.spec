...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 6667
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 6666
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 16667
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 16666
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 6667
	%{_sbindir}/semanage port -d -p tcp 6666
	%{_sbindir}/semanage port -d -p tcp 16667
	%{_sbindir}/semanage port -d -p tcp 16666
	for selinuxvariant in %{selinux_variants}; do
