...
*Setup instructions for a Gitian build of 1Credit Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the 1Credit
...
- [Building 1Credit Core](#building-1credit-core)
...
Clone the git repositories for 1credit and Gitian.
...
in the 1credit repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building 1Credit Core
----------------

To build 1Credit Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the 1credit repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/1credit/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/1credit-project/1credit
...
For example:
```bash
URL=https://github.com/thrasher-/1credit.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit 1credit=${COMMIT} --url 1credit=${URL} ../1credit/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit 1credit=${COMMIT} --url 1credit=${URL} ../1credit/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit 1credit=${COMMIT} --url 1credit=${URL} ../1credit/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the 1credit git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../1credit/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/1credit
SIGPATH=/some/root/path/1credit-detached-sigs

./bin/gbuild --url 1credit=${BTCPATH},signature=${SIGPATH} ../1credit/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/1credit-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/1credit-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/1credit-osx-build.assert
```
...
[1credit-project/gitian.sigs.1cr](https://github.com/1credit-project/gitian.sigs.1cr/) repository, or if that's not possible create a pull
