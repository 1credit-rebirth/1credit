1Credit Core version 0.13.2 is now available from:

  <https://download.1credit.org/1credit-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/1credit-project/1credit/issues>
...
updates will be released anymore. Without security updates, using a 1credit
...
ECDSA signatures inside 1Credit transactions now use validation using
...
Previous versions of 1Credit Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of 1Credit Core was to raise the default minimum
relay fee.

1Credit Core 0.13.2 will have a strict maximum size on the mempool. The
...
1Credit Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running 1creditd without having to do any manual configuration.
...
1Credit Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. 1Credit Core will relay transactions with
...
entirely in 1Credit Core 0.13, and it is at this time undecided whether the
...
1Credit Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
1Credit Core automatically creates a hidden service to listen on, without
manual configuration. 1Credit Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if 1Credit Core is listening, and
...
1Creditd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals 1Credit
Core to use floating fees. By default, 1Credit Core will use floating
fees.
...
(default: `0.0002` 1CR/kB).

At all times, 1Credit Core will cap fees at `-maxtxfee=<x>` (default:
0.10) 1CR.
Furthermore, 1Credit Core will never create transactions paying less than
...
`1credit.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 26667 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 26667 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /1creditrpc>
        ...
        # optional bypass 1creditd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< 1creditrpc:<password>
    </Location>
...
Restarting 1creditd will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in 1credit.conf
...
For example:
    ...
    $ src/1credit-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The 1Credit Core code base has
...
- `1credit-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `1credit-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the 1Credit scripting system that in combination with
...
1Credit transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of 1Credit Core.
...
The two phases are distinct in the 1Credit-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming 1Credit wallet
  software more difficult and which seriously complicates the design of smart
  contracts for 1Credit.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each 1Credit block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of 1credits they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of 1credits being spent is exactly
...
- **Increased security for multisig:** 1Credit addresses (both P2PKH addresses
  ...
  believed to be provided by 1Credit's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of 1Credit's current ledger (called the UTXO set),
  ...
  note that 1Credit Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  1Credit users to individually opt-in to almost any change in the 1Credit
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and 1Credit Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the 1Credit
client to 1Credit Core 0.13.2 to signal for segwit support. When a
...
Since 1Credit Core 0.10.0, nodes have defaulted to only relaying and
...
- `1credit-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `1credit-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
